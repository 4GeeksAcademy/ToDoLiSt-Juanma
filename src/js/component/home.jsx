...
const Home = () => {
	...
	const guardarTareas = async () => {
		try {
			const url = "https://playground.4geeks.com/todo/todos/juanma"
	...
	const cargarTareas = async () => {
		try {
			const url = "https://playground.4geeks.com/todo/users/juanma"
			const resp = await fetch(url)
			if (resp.status == 404) {
				crearUsuario()
				return
	...
	const crearUsuario = async () => {
		try {
			const resp = await fetch("https://playground.4geeks.com/todo/users/juanma", {
				method: "POST",
				headers: { "Content-Type": "application/json" }
			})
			if (resp.status == 201) {
